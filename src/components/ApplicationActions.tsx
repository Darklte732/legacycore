...
interface ApplicationActionsProps {
  ...
  role?: 'agent' | 'manager'; // New prop to determine paths
}

export function ApplicationActions({ applicationId, role = 'agent' }: ApplicationActionsProps) {
  ...
  const basePath = role === 'manager' ? '/manager/applications' : '/agent/applications';
  
  // Get the user's role from Supabase on component mount
  useEffect(() => {
  ...
  const viewApplicationDetails = () => {
    ...
    router.push(`${basePath}/${applicationId}`)
  }

  const editApplication = () => {
    console.log("Editing application:", applicationId, "with role:", role);
    // Use direct navigation for more reliable navigation
    window.location.href = `${basePath}/${applicationId}/edit`;
  }
        ...
        <DropdownMenuContent align="end" className="w-56 bg-white shadow-lg z-50 border border-gray-200">
          <DropdownMenuItem 
            onClick={() => {
              ...
              window.location.href = `${basePath}/${applicationId}`;
            }} 
